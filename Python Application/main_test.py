...
class YOLOApp(tk.Tk):
    def __init__(self, font_family="Poppins"):
        ...
        #
        # Linha VERTICAL: quando o borde direito (x2) de um lock cruzar
        # esta linha, aguarda TRIP_DELAY_MS milissegundos e dispara 'd'.
        #
        # trip_line_ratio: posição X da linha como fração da largura do frame
        #   ex.: 0.85 = linha a 85% da largura (perto da borda direita).
        # Os locks que já dispararam ficam no conjunto _tripped_locks para
        # não disparar mais de uma vez por objeto.
        # ─────────────────────────────────────────────────────────────────────
    ...
    def _build_left(self):
        ...
        self._section_tripline(inner2)   # ← nova seção
    ...
    def _loop(self):
        while self.running:
            ...
            # Atualiza dimensões reais do frame
            fh, fw = frame.shape[:2]
            self._frame_h = fh
            self._frame_w = fw
            ...
            # ── Desenha a trip line vertical sobre o frame anotado ────────────
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  MODIFICAÇÃO 1 — _is_defect_class agora inclui defeitos de RÓTULO
    # ══════════════════════════════════════════════════════════════════════════
    def _is_defect_class(self, cls_name):
        """
        Retorna True para classes que indicam defeito.
        Inclui defeitos de tampa/lacre/nível E defeitos de rótulo/etiqueta.
        """
        name = cls_name.lower().strip()
        defect_keywords = (
            # ── defeitos de tampa / lacre / nível (originais) ──
            "tampa", "lacre",
            "nivel", "nível",
            "liquido", "líquido",
            "level", "cap", "seal",
            # ── NOVO: defeitos de rótulo / etiqueta ──────────────
            "rotulo", "rótulo",
            "etiqueta",
            "label",
            "logo",
    ...
    def _handle_serial_trigger(self, result, n_det):
        ...
        Dispara 'd' quando um defeito confirmado sai de cena por completo.
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  MODIFICAÇÃO 2 — TRIP LINE: disparo quando lock cruza a linha
    # ══════════════════════════════════════════════════════════════════════════

    def _draw_trip_line(self, frame, trip_x):
        """
        Desenha a linha de disparo VERTICAL sobre o frame.
        Cor: vermelho sólido; muda para amarelo quando há disparo pendente.
        """
        h, w = frame.shape[:2]
        out = frame

        LINE_COLOR  = (0, 60, 220)    # BGR — vermelho vivo
        FLASH_COLOR = (0, 220, 255)   # BGR — amarelo (lock pendente)
        ...
        tx = trip_x + 6
        ty = h // 2
        # Se a linha está muito perto da borda direita, coloca o texto à esquerda
        if tx + tw + 8 > w:
            tx = trip_x - tw - 12
    ...
    def _check_trip_line(self, trip_x):
        """
        Verifica se algum lock ativo cruzou a trip line VERTICAL (x2 >= trip_x).

        Fluxo por lock:
          1. Se lock.id NÃO está em _tripped_locks e x2 >= trip_x:
               registra timestamp em _pending_trips (início do delay).
          2. A cada frame, verifica _pending_trips:
               se (agora - timestamp) >= TRIP_DELAY_MS/1000 → dispara 'd'.
          3. Após disparar, move id para _tripped_locks (não dispara mais).
          4. Quando o lock é liberado (sai de _locks), remove de _tripped_locks
             para que um novo objeto possa disparar.
        """
        now = time.time()
        active_ids = {l["id"] for l in self._locks}

        # ── Marca novos locks que cruzaram a linha ────────────────────────────
        for lock in self._locks:
            lid = lock["id"]
            x2  = lock["box"][2]   # borde direito do box
        ...
        # ── Processa disparos pendentes cujo delay já expirou ─────────────────
        fired = []
        ...
        # ── Limpa _tripped_locks de IDs que já sumiram dos locks ativos ───────
    ...
    def _annotate_lock(self, frame, trip_x=None):
        ...
        Quando um lock ainda não disparou e x2 já passou de trip_x,
        usa cor laranja de alerta em vez do magenta padrão.
        """
        if not self._locks:
            return frame
        out = frame

        LOCK_COLOR    = (220, 80, 255)   # BGR magenta — normal
        LOCK_COLOR2   = (180, 40, 200)
        ALERT_COLOR   = (0, 140, 255)    # BGR laranja — cruzou a linha, aguardando
        ...
        for lock in self._locks:
            ...
            # Cor de alerta se borde direito cruzou a linha e ainda não disparou
            ...
            # Barra de progresso horizontal abaixo do box durante o delay
    ...
    def _is_ignored_class(self, cls_name):
        """Retorna True para classes que devem ser completamente ignoradas."""
        name = cls_name.lower().strip()
